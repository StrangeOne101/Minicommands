			...
			if (c == '(' && (i == 0 || line.charAt(i - 1) != '\\') && state == ChatParseState.NONE) { 
				...
				for (BaseComponent component : componentLine) {
					if (!enclosedHover.equals("")) {
						component.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new BaseComponent[] {new TextComponent(GenericUtil.lengthSplit(enclosedHover, 60))}));
					} 
					...
					if (!enclosedCommand.equals("")) {
						...
						} else if (enclosedCommand.startsWith("!")){
							component.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, enclosedCommand.substring(1)));
						} else {
